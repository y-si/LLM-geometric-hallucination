...
\paragraph{Mixture-of-Experts architectures.} Both of our intervention target models use Mixture-of-Experts (MoE) routing, in which only a subset of parameters is active for each token. Mixtral 8x7B has 46.7 billion total parameters with approximately 12.9 billion active per token \citep{jiang2024mixtral}; Llama 4 Maverick routes among 128 experts with approximately 17 billion active parameters per token. MoE architectures achieve strong performance at lower inference cost than equivalently-sized dense models, and their use of distinct experts for different inputs provides architectural diversity between our two target models (Chapter~\ref{ch:setup}).
...
The geometric analysis in this thesis operates on \emph{text embeddings}---fixed-dimensional vector representations of natural language. This section introduces the embedding space, the distance metric we use, and the geometric concepts that our features (defined precisely in Chapter~\ref{ch:setup}) formalize.
...
\label{eq:cosine-sim}
\end{equation}
and the cosine distance is $d_{\cos}(\mathbf{x}, \mathbf{y}) = 1 - \text{sim}(\mathbf{x}, \mathbf{y})$, ranging from 0 (identical direction) to 2 (opposite direction). Cosine distance is invariant to vector magnitude and depends only on the angle between vectors, making it appropriate for embeddings where direction carries more semantic information than magnitude. When vectors are L2-normalized (as in our setting), cosine distance and squared Euclidean distance are related by $\|\mathbf{x} - \mathbf{y}\|^2 = 2\,(1 - \text{sim}(\mathbf{x}, \mathbf{y}))$, so the choice of metric is equivalent.

\subsection{High-Dimensional Geometry}
\label{sec:bg-highdim}

Our embeddings reside in $\mathbb{R}^d$ for $d = 3{,}072$ (the dimensionality of the embedding model selected in Chapter~\ref{ch:setup}). High-dimensional spaces exhibit counterintuitive geometric properties that affect how we interpret distance-based features. A well-known example is \emph{distance concentration}: pairwise distances between random points in high dimensions converge to a narrow range, making absolute distances less informative \citep{aggarwal2001surprising}. Within structured datasets, however, relative distances---which points are closer or farther than others---remain meaningful. All of our geometric features (Chapter~\ref{ch:setup}) are defined relative to the corpus distribution rather than in absolute terms, making them robust to concentration effects. When embeddings are L2-normalized, all points lie on the surface of the unit hypersphere in $\mathbb{R}^{3072}$, and cosine distance is the natural metric on this surface.
...
The geometric features defined in Chapter~\ref{ch:setup} each formalize a different aspect of manifold structure. Before those precise definitions, we introduce the underlying concepts.

\paragraph{$k$-nearest neighbors.} For a point $\mathbf{x}_i$ in a dataset of $N$ points, its $k$-nearest neighbors ($k$-NN) are the $k$ points with smallest distance to $\mathbf{x}_i$ under the chosen metric. The $k$-NN set is the building block for most of our geometric features: it defines the ``local neighborhood'' within which we measure density, dimensionality, and curvature. The choice of $k$ controls the locality of the analysis: small $k$ captures fine-grained structure; large $k$ smooths over broader neighborhoods.

\paragraph{Principal Component Analysis.} Principal Component Analysis (PCA) finds the orthogonal directions of maximum variance in a dataset---the eigenvectors of the covariance matrix, ordered by eigenvalue magnitude. The \emph{explained variance ratio} of the top $p$ components,
\begin{equation}
\text{EVR}(p) = \frac{\sum_{j=1}^{p} \sigma_j^2}{\sum_{\ell} \sigma_\ell^2},
\label{eq:evr}
\end{equation}
where $\sigma_j^2$ are the eigenvalues, measures how well a $p$-dimensional linear subspace approximates the data. PCA appears in two roles in our work: measuring how well a flat subspace approximates a local neighborhood (curvature), and constructing a novel ``semantic opposite'' of each point (oppositeness). Both are defined precisely in Chapter~\ref{ch:setup}.

\paragraph{Intrinsic dimensionality.} The \emph{intrinsic dimension} of a dataset at a point estimates the number of effective directions of variation in the local neighborhood. A manifold embedded in 3,072-dimensional space may have far fewer effective local dimensions. Standard estimators include the TwoNN method \citep{facco2017twonn}, which uses only the ratio of distances to the two nearest neighbors, and the maximum likelihood estimator of \citet{levina2004maximum}, which averages log-distance ratios over a larger neighborhood for a smoother estimate.

\paragraph{Local density.} The \emph{density} at a point measures how many similar points surround it---equivalently, the inverse of the average distance to nearby neighbors. High density indicates a well-represented region with many semantically similar texts; low density indicates an isolated point with few nearby neighbors, potentially representing out-of-distribution or under-represented content. Density-based measures are standard in anomaly detection \citep{breunig2000lof}.

\paragraph{Curvature.} The \emph{curvature} of the manifold at a point reflects how well a flat (linear) subspace approximates the local neighborhood. If the neighborhood lies on a flat subspace, PCA captures most of the variance and the residual is small (low curvature). If the neighborhood bends or twists, the linear approximation misses variance and the residual is large (high curvature).

\paragraph{Centrality.} The \emph{centrality} of a point, as used in this thesis, measures its distance from the center of the distribution (e.g., the corpus mean embedding); higher values indicate more \emph{peripheral} points. Points far from the center may represent unusual, specialized, or boundary-region content.

\paragraph{Oppositeness.} In addition to these standard geometric concepts, Chapter~\ref{ch:setup} defines a novel \emph{oppositeness} feature that measures how far a point's ``semantic opposite'' (constructed by flipping principal component directions) lies from the nearest real data point. The construction is specific to our experimental design and is defined precisely in Section~\ref{sec:oppositeness}.

\section{Evaluating Language Model Outputs}
\label{sec:bg-evaluation}

Evaluating whether an LLM response is factually correct, partially correct, hallucinated, or a refusal requires semantic judgment that scales beyond traditional automated metrics and human annotation. This section introduces the LLM-as-judge paradigm that our evaluation pipeline builds on.
...
Chapter~\ref{ch:finetuning} presents experiments in which we fine-tune language models to reduce hallucination. This section introduces the relevant concepts.
