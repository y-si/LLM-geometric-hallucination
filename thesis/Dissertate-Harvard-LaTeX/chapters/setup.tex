...
This chapter describes the experimental infrastructure shared across all experiments in this thesis: the hallucination benchmark (Section~\ref{sec:benchmark}), the language models under study (Section~\ref{sec:models}), the consensus judging pipeline that converts model responses into hallucination labels (Section~\ref{sec:judging}), and the embedding approach that enables geometric analysis (Section~\ref{sec:embeddings}). Experiment-specific methods---geometric feature extraction, prompt prefix design, and fine-tuning procedures---are introduced in their respective chapters.
...
\paragraph{Factual.} Prompts with unambiguous, objectively verifiable answers grounded in established knowledge. Examples include ``What is the chemical formula for sodium chloride?'' and ``Who wrote the book \emph{1984}?'' These serve as a baseline control, analogous to the factual recall questions in TruthfulQA \citep{lin2022truthfulqa} and HaluEval \citep{li2023halueval}. Hallucination on such factual prompts reflects a clear failure of knowledge recall. In the expanded benchmark, this category contains 500 prompts drawn from 56 templates across 22 entity pools covering countries, chemical compounds, historical events, literary works, and scientific facts.

\paragraph{Nonexistent.} Prompts referencing entities, works, or concepts that do not exist. Examples include ``Summarize the book \emph{The Obsidian Codex} by Dr. Marcus Valmont'' and ``Explain the Bertrand-Goldstein theorem in mathematics.'' The correct response is to identify the entity as nonexistent; any detailed description of the entity constitutes hallucination. This category operationalizes the question of whether models ``know what they don't know'' \citep{kadavath2022language, yin2023large}. This is the largest category (600 prompts in the expanded benchmark) because it targets the failure mode most central to hallucination: fabricating information about entities that do not exist. Unlike factual prompts, where correct information may be available in the model's training data, nonexistent prompts offer no correct content to recall---any detailed response necessarily constitutes fabrication. The category draws from 55 templates and 34 entity pools of fabricated names, theorems, books, countries, inventions, and other entities.

\paragraph{Impossible.} Prompts requesting tasks that cannot be completed by any system: proving unsolved conjectures, predicting future events with certainty, reading the user's thoughts, or listing infinite sets. Examples include ``Provide a complete proof of the Birch and Swinnerton-Dyer conjecture'' and ``Tell me exactly what I'm thinking right now.'' Unlike nonexistent prompts, which test entity knowledge, impossible prompts test whether models recognize the \emph{logical or physical} limits of answerable questions. This category contains 200 prompts in the expanded benchmark, drawn from 66 templates (20 of which are static, single-use prompts such as ``Prove that P $=$ NP'').

\paragraph{Ambiguous.} Prompts with no single correct answer, where the response legitimately depends on subjective criteria, values, or framing. Examples include ``What makes a good leader?'' and ``Is it foolish to lie to protect someone?'' The correct behavior is to acknowledge the ambiguity rather than present a subjective opinion as fact. Fabricating verifiable claims (e.g., citing a nonexistent study) within an otherwise subjective response still constitutes hallucination. The precise boundary between acceptable subjectivity and hallucination is operationalized through the consensus judging pipeline described in Section~\ref{sec:judging}. This category contains 600 prompts in the expanded benchmark, drawn from 55 templates and 31 entity pools (37 variables total, with paired variables for comparative templates counted as single pools) including professions, abstract concepts, and paired comparison items.

\paragraph{Borderline: Plausible Fake.} Entities that sound real but do not exist---the adversarial counterpart to nonexistent prompts, probing the region just beyond the tail of \citet{sun2024entity}'s entity popularity spectrum---entities plausible enough to be real but that do not actually exist. Where nonexistent entities are often transparently fabricated (e.g., the country ``Quantia,'' the disease ``Chromatic Resonance Fatigue''), plausible fakes are designed to be difficult to distinguish from real entities: ``Dr.\ Helena Vasquez-Okonkwo,'' ``\emph{The Lintel Above the Dovecote},'' ``Penderwych Narrows.'' We expect this to be the hardest category in our benchmark: unlike transparently fake entities, plausible fakes cannot be rejected on surface features alone, forcing the model to rely on genuine knowledge of whether the entity exists. This category contains 200 prompts in the expanded benchmark across three entity subtypes (people, books, places), each with 45 fabricated entities and 20 templates.

\paragraph{Borderline: Obscure Real.} Real entities that are sufficiently obscure that a model might incorrectly deny their existence---the ``tail'' entities in the terminology of \citet{sun2024entity}, where model knowledge degrades sharply. Examples include Rosalind Franklin (who is often underattributed), Tristan da Cunha (the world's most remote inhabited island), and the Cadaver Synod (a medieval papal trial of a corpse). These prompts test the opposite failure mode from plausible fakes: \emph{false negatives} on entity existence, where a model refuses to answer about something that genuinely exists. The category contains 200 prompts in the expanded benchmark across three subtypes (people, places, events) with 39--40 verified entities each and 20 templates per subtype.

\paragraph{Borderline: Edge Factual.} Factual questions designed to elicit confidently wrong answers---commonly confused facts, counter-intuitive truths, and widespread misconceptions. Examples include ``What is the largest desert in the world?'' (Antarctica, not the Sahara, which is a common misconception arising from the colloquial association of ``desert'' with hot, sandy terrain, whereas the geographic definition includes any region with very low precipitation), ``What is the capital of Turkey?'' (Ankara, not Istanbul), and ``What is the probability of two people in a group of 23 sharing a birthday?'' (approximately 50.7\%, which most people estimate as much lower). Unlike standard factual prompts, edge factual prompts target knowledge boundaries where models are likely to reproduce common human errors---what \citet{lin2022truthfulqa} term ``imitative falsehoods,'' where models learn to repeat popular misconceptions from training data. This category contains 130 prompts in the expanded benchmark, each individually curated with a verified ground truth and an explanatory note documenting the expected error pattern.
...
\paragraph{Entity filling.} When a template contains multiple placeholders (e.g., \texttt{"Who is \{fake\_name\}, the inventor of \{fake\_invention\}?"}), each variable is filled independently from its respective entity pool. For paired variables that must be semantically coherent (e.g., \texttt{\{option1\}} and \texttt{\{option2\}} in comparison templates), we use structured entity pools where each entry is a dictionary of related values, and a pair-resolution step maps sub-keys to their parent pool. This mechanism was added after discovering that 6 ambiguous templates with paired variables produced 108 unfilled placeholders in an initial generation pass for the expanded benchmark; the fix raised ambiguous template coverage from 89.1\% to 100\%.
...
\paragraph{Fake entity validation.} Plausible fake entities pose a particular validation challenge: an entity designed to sound real might \emph{actually be real}. We conducted a two-stage validation:
\begin{enumerate}
    \item \textbf{Automated sweep}: All 135 fake entities (45 people, 45 books, 45 places) were searched against web sources for real-world collisions. This flagged 27 entities with potential real-world collisions: 6 people with exact name matches to real individuals, 15 books sharing titles with published works (generic literary titles like ``The Dry Season'' almost always collide), and 6 places matching real locations (e.g., ``Blackfen'' is a real neighborhood in London, ``Lake Meridian'' is a real lake in Washington state).
    \item \textbf{Human review}: All 27 flagged entities were manually reviewed. Three clear collisions were replaced with compound-surname names and invented proper nouns designed to minimize collision risk. The remaining 24 were retained after review judged the real-world matches unlikely to affect model behavior based on their minimal web presence (e.g., a person sharing a name with a minor historical figure with no online footprint) or determined them to be false positives upon closer inspection.
\end{enumerate}
...
The benchmark defines \emph{what} we test. The next section describes \emph{who} we test: the language models selected for evaluation across the experimental phases of this thesis.
...
\begin{tabular}{llll}
\toprule
\textbf{Model} & \textbf{Provider} & \textbf{Architecture} & \textbf{Access} \\
\midrule
GPT-5.1 & OpenAI & Dense & API \\
GPT-4.1 & OpenAI & Dense & API \\
GPT-4.1-mini & OpenAI & Dense & API \\
GPT-4o-mini & OpenAI & Dense & API \\
\midrule
Claude Opus 4.5 & Anthropic & Dense & API \\
Claude Sonnet 4.5 & Anthropic & Dense & API \\
Claude Haiku 4.5 & Anthropic & Dense & API \\
\midrule
Llama 4 Maverick 17B & Meta (Together AI) & MoE & Open-weight \\
Mixtral 8x7B & Mistral (Together AI) & MoE & Open-weight \\
Qwen 3 Next 80B & Alibaba (Together AI) & MoE & Open-weight \\
\bottomrule
\end{tabular}
\caption{The ten models in the cross-model benchmark suite. Models span three provider families, multiple capability tiers within each family, and both dense and Mixture-of-Experts architectures. API-only models are evaluated via their respective provider APIs; open-weight models are served through Together AI. All models use their instruction-tuned variants.}
\label{tab:models}
\end{table}

\paragraph{Selection rationale.} The three provider families---OpenAI, Anthropic, and open-weight---were chosen to minimize the risk that geometric patterns reflect a single training pipeline's biases. Within each family, we include models at different capability tiers: GPT-5.1 alongside GPT-4o-mini, Claude Opus 4.5 alongside Claude Haiku 4.5. If geometric features predict hallucination only for the strongest models (or only for the weakest), the signal would be less interesting than if it holds across capability scales. The three open-weight models introduce architectural diversity: Mixtral 8x7B is a sparse MoE with 8 experts (2 active per token), Llama 4 Maverick is a large-scale MoE with 128 experts (17B parameters active per token), and Qwen 3 Next 80B provides a third independently trained MoE architecture. Whether MoE routing alters the geometric structure of embedding space---and whether this affects the predictive power of our features---is an empirical question that this model set is designed to answer.

\paragraph{Limitations of model selection.} We note several constraints. First, API-only models do not disclose parameter counts, which prevents us from reporting a clean scaling analysis. Second, model behavior changes over time as providers update their systems; the cross-model benchmark results in Chapter~\ref{ch:geo-predict-hallucination} reflect the most recent model versions available at the time of evaluation (Fall 2025), and the geometric prediction framework is architecture-agnostic and can be applied to newer models as they become available. Third, ten models cannot exhaustively represent the space of all architectures---notably, we do not include any models from Google (Gemini) or any non-transformer architectures (e.g., state-space models). Fourth, in our benchmark suite, all open-weight models happen to use MoE architectures while all API-only models are dense, which means architecture type is partially confounded with access type. This does not affect the geometric analysis---which uses a shared external embedding model (Section~\ref{sec:embeddings}) rather than model-internal representations---but it does mean we cannot cleanly isolate the effect of MoE routing on hallucination behavior from other differences between provider families. The set was chosen to be representative rather than exhaustive, balancing diversity against the practical cost of running the full evaluation pipeline (449 prompts $\times$ 3 judges per model).

\subsection{Intervention Target Models}
\label{sec:target-models}

The fine-tuning experiments in Chapter~\ref{ch:finetuning} require parameter-efficient fine-tuning via LoRA \citep{hu2022lora}, which modifies a model's weight matrices and therefore requires access to model weights. API-only models do not support weight-level modification, restricting the fine-tuning experiments to open-weight models. We conduct the prompting experiments (Chapter~\ref{ch:prefixes}) on the same two models so that prompting and fine-tuning results are directly comparable on identical model--prompt pairs---the central question of Chapter~\ref{ch:finetuning} is whether fine-tuning can match or exceed the best prompt prefix, which requires that both interventions be evaluated on the same base model.

From the three open-weight models in our benchmark suite, we select two: \textbf{Mixtral 8x7B} and \textbf{Llama 4 Maverick 17B}. We use the instruction-tuned variants of each (\texttt{Mixtral-8x7B-Instruct-v0.1} and \texttt{Llama-4-Maverick-17B-128E-Instruct}; for inference, the FP8-quantized variant of Llama is used for efficiency, while fine-tuning uses the full-precision checkpoint) because instruction-tuned models accept system prompts directly, which is essential for the prompting interventions. Using base models would require few-shot formatting, introducing a confound between the prompt template and the intervention itself.

\paragraph{Why two models.} Two architecturally distinct models provide a minimal but sufficient test of whether interventions generalize beyond a single architecture. If a prompting strategy reduces hallucination on Mixtral but not Llama (or vice versa), the intervention is architecture-specific. If it works on both, we have evidence of generality across at least two distinct MoE designs. We do not claim that two models establish universal generality---only that they are sufficient to distinguish interventions that are genuinely effective from those that exploit a single model's idiosyncrasies. Extending to all three open-weight models (including Qwen 3 Next 80B) would strengthen this claim but would triple the cost of the prompting and fine-tuning experiments, which require tens of thousands of judge evaluations per model.

\paragraph{Architectural contrast.} The two models differ in both architecture and scale. Mixtral 8x7B is a sparse MoE with 8 expert feed-forward networks per layer, of which 2 are activated per token, yielding approximately 12.9 billion active parameters from a total of 46.7 billion. Llama 4 Maverick is a substantially larger MoE with 128 experts per layer, activating 17 billion parameters per token from a total of approximately 400 billion. Beyond the quantitative differences in expert count and active parameters, the two models were trained by different organizations (Mistral AI and Meta, respectively) with different training data, objectives, and instruction-tuning procedures. This ensures that any shared patterns that emerge---prompts that both models hallucinate on, or interventions that reduce hallucination for both---are unlikely to reflect idiosyncratic training artifacts.

\paragraph{Why not more models.} Cross-model generality of geometric \emph{predictors} is already established by the ten-model benchmark in Chapter~\ref{ch:geo-predict-hallucination}. The intervention experiments in Chapters~\ref{ch:prefixes} and~\ref{ch:finetuning} address a different question: whether prompting and fine-tuning can \emph{reduce} hallucination, and whether geometric features explain which prompts are amenable to intervention. This question requires depth (multiple intervention strategies, statistical testing on paired data, fine-tuning with hyperparameter search) rather than breadth across many models. Two models with contrasting architectures and different expected difficulty levels---given their differences in scale and training---provide a principled minimal design.

\subsection{Generation Parameters}
\label{sec:generation}

All model responses throughout this thesis---across baseline evaluation, prompting experiments, and fine-tuned model evaluation---are generated with the same inference parameters:

\begin{itemize}
    \item \textbf{Temperature}: 0.7. This is standard for open-ended generation tasks, balancing response diversity with coherence. A temperature of 0 would produce deterministic outputs but would be unrepresentative of real deployment settings, where stochastic sampling is the norm.
    \item \textbf{Maximum tokens}: 4,000. This upper bound is generous enough to avoid truncation artifacts on any prompt in our benchmark. In practice, most responses are substantially shorter.
    \item \textbf{No additional constraints}: We do not set top-$p$, top-$k$, or other sampling parameters, allowing the full distribution at the specified temperature.
\end{itemize}

Holding these parameters constant across all experimental conditions ensures that any observed differences in hallucination rates are attributable to the intervention (prompt prefix or fine-tuning), not to changes in the sampling procedure.
...
\paragraph{Single-sample design.} Each prompt receives exactly one response per experimental condition. We do not resample or average across multiple generations for the same prompt. This is a deliberate design choice: the practical question motivating this thesis is whether single-shot interventions (a system prompt, or a fine-tuned model) reliably shift behavior on individual prompts, which is the relevant deployment scenario. Statistical power comes from the breadth of the benchmark (2,879 prompts) rather than from repeated sampling of individual prompts. A multi-sample design would increase the cost of each experimental condition by a factor equal to the number of samples, without changing the fundamental experimental question.

% Transition to next section
The model selection defines the \emph{who}; the next section describes how we \emph{evaluate}---the consensus judging pipeline that converts raw model responses into hallucination labels.
...
LLM judges are known to exhibit systematic biases, including position bias (favoring responses presented first in pairwise comparisons), verbosity bias (favoring longer responses), and self-preference bias---a tendency to assign higher scores to outputs generated by the same model family \citep{wataoka2024selfpreference}. We address these concerns through three design decisions: (1) a multi-model panel that reduces any single model's bias (Section~\ref{sec:panel}), (2) a structured rubric with category-specific rules that constrains evaluation to factual accuracy rather than stylistic preferences (Section~\ref{sec:rubric}), and (3) human validation to ground-truth the automated pipeline (Section~\ref{sec:human-validation}). The effectiveness of these mitigations is evaluated empirically in Chapter~\ref{ch:geo-predict-hallucination}.
...
\paragraph{Why three judges.} An odd number of judges enables majority voting without ties (Section~\ref{sec:majority-vote}). Three is the minimum panel size that provides meaningful consensus while remaining cost-effective: a single judge risks systematic bias that cannot be detected without a second opinion, while five judges would increase the number of API calls by 67\% with diminishing returns on reliability---the marginal information from a fourth and fifth judge is smaller than from the second and third, since three judges already span three independent model families.
...
\paragraph{Judge--evaluatee overlap.} One judge (Llama 4 Maverick) also serves as an intervention target model. This creates a potential self-preference bias: \citet{wataoka2024selfpreference} show that LLMs tend to assign higher scores to outputs with lower perplexity under their own distribution, which includes their own outputs. We acknowledge this as a limitation, mitigated by two factors. First, Llama's judgment is one of three votes; it cannot unilaterally determine the consensus label. Second, we conduct a sensitivity analysis (Section~\ref{sec:sensitivity}) that specifically examines whether Llama's labels differ systematically when evaluating its own outputs versus those of other models.
...
\paragraph{Why four labels, not binary.} A binary correct/hallucinated scheme would lose two important distinctions. First, \emph{partial} responses (label 1) represent cases where the model demonstrates relevant knowledge but falls short---through vagueness, minor inaccuracies, or missing the core point of the question. These are qualitatively different from full hallucinations (label 2), where the model fabricates or contradicts facts, and they carry different implications for intervention design. A model that responds partially to a prompt may be recoverable through targeted prompting or fine-tuning, while a model that fully fabricates may lack the underlying knowledge entirely. Second, \emph{refusals} (label 3) are mechanistically distinct from hallucinations: a model that says ``I don't know'' has, in some sense, succeeded at metacognition, even if the response is unhelpful. Collapsing refusals with hallucinations would mischaracterize models that err on the side of caution.

\paragraph{Category-specific evaluation rules.} The four-point rubric interacts with the seven prompt categories defined in Section~\ref{sec:categories}. A response that would be ``correct'' for a factual prompt might constitute a hallucination for a nonexistent entity prompt, and vice versa. The seven prompt categories map onto four evaluation rule types, since categories that share the same correct-response logic are evaluated under the same rule: nonexistent and plausible fake entities share the ``entity does not exist'' rule; factual, edge factual, and obscure real prompts share the factual accuracy rule; impossible prompts and ambiguous prompts each have their own rule. The judge system prompt presents all four labels and all category-specific rules simultaneously; the judge assigns a single label in one step, not through a sequential process. The prompt is organized into two components. The first provides \textbf{category-specific rules} that define what \emph{Correct} and \emph{Hallucinated} mean for each category type, since the boundary between these labels differs fundamentally across categories:
...
The second component provides guidance for the remaining two labels---\emph{Partial} and \emph{Refused} (defined in Table~\ref{tab:rubric})---which apply uniformly across all categories. For example, a response to a nonexistent entity prompt that expresses uncertainty but does not clearly identify the entity as fake would be \emph{Partial}, while a bare ``I cannot answer that'' without further explanation would be \emph{Refused}. The key distinction between \emph{Refused} and a \emph{Correct} response to an impossible or nonexistent prompt is whether the model demonstrates substantive understanding or simply declines.

This prompt structure is important for evaluation consistency. Without the category-specific rules, a model that correctly responds ``This theorem does not exist'' to a nonexistent entity prompt could be misclassified as a refusal rather than a correct response, since the surface form resembles a declination. Without the general labels, judges would be forced into a binary Correct/Hallucinated choice even when the response falls in between.

\subsection{Judge Prompt Design}
\label{sec:judge-prompt}

Each judge receives a system prompt containing the complete evaluation rubric (including the category-specific rules described above) and a user prompt containing three fields:
...
\paragraph{Ground truth provision.} Providing the ground truth to judges is a deliberate design decision. Our task is to evaluate \emph{factual accuracy}---whether the model's response aligns with known facts---not to assess open-ended response quality. This makes our evaluation closer to reference-based metrics (e.g., comparing against a gold standard) than to preference-based evaluation (e.g., choosing between two responses). Providing the ground truth reduces the subjectivity of evaluation and ensures that judges have the information needed to detect subtle hallucinations, such as a model providing mostly correct information about an obscure real entity but fabricating one specific detail.

\paragraph{No few-shot examples.} The system prompt relies on the rubric and category-specific rules alone, without few-shot examples. This is a deliberate choice: few-shot examples risk anchoring judges to specific response patterns, potentially biasing evaluation toward or against responses that resemble the examples. The rubric is designed to be sufficiently precise that examples are unnecessary. Whether this design succeeds is an empirical question evaluated through the human validation study (Section~\ref{sec:human-validation}).

\paragraph{Structured output.} The OpenAI and Together AI APIs are called with \texttt{response\_format: json\_object}, which constrains the output to valid JSON. The Anthropic API does not support this constraint natively, so an additional instruction to respond in JSON is appended to the user prompt. In all cases, the response is parsed and validated: missing keys, out-of-range labels, or malformed JSON trigger a retry (up to three attempts with exponential backoff). The full system prompt text is reproduced in Appendix~\ref{app:judge-prompt}.
...
\end{equation}
%
where $n$ is the number of judges that returned valid responses (typically $n = 3$; reduced when a judge fails, as described below) and $c_i$ is judge $i$'s self-reported confidence. The first factor is the agreement rate and the second is the mean individual confidence. This formulation penalizes cases where judges disagree: a split decision (e.g., $2/3$ agreement when all judges succeed) with high individual confidence still receives a lower consensus confidence than a unanimous decision with moderate individual confidence. The consensus confidence is used for downstream filtering in cases where high-confidence labels are needed (e.g., selecting training data for fine-tuning).

\paragraph{Parallelization.} The three judge calls are executed in parallel to reduce latency. Results are collected in a fixed order matching the panel ordering, ensuring that the tiebreaking behavior described above is deterministic regardless of which judge responds first.
...
% [RESOLVED] BIG FLAG: COME BACK TO THIS AFTER CLEANING CONTAMINATION
...
% FLAG: extra space?
\paragraph{Known limitations.} Beyond the validation limitations noted above, the consensus judging pipeline has several known limitations that we acknowledge transparently:

\begin{itemize}
    \item \textbf{Shared blind spots}: All three judge models are large language models trained on broadly similar internet text. They may share systematic blind spots---for example, all failing to detect a hallucination that involves an obscure fact absent from all three models' training data. The ground truth provision mitigates this partially, but judges must still correctly interpret the ground truth.
    \item \textbf{Rubric sensitivity}: The category-specific rules were designed by the thesis author. Alternative rule formulations (e.g., a stricter definition of ``partial'' or a different treatment of hedged responses) might produce different label distributions. We do not claim that our rubric is the only defensible one, only that it is internally consistent and validated against human judgment.
    \item \textbf{Determinism}: Temperature $= 0.0$ reduces but does not eliminate stochasticity. API-level implementation details (e.g., floating-point non-determinism in batched inference) can produce occasional variation across runs on identical inputs, though the effect is expected to be small.
\end{itemize}

% Transition to next section
The judging pipeline defines how we \emph{label} model responses. The final component of the shared infrastructure is the embedding approach that enables geometric analysis of prompts.
...
We embed all 2,879 benchmark prompts---449 from the held-out test set and 2,430 from the training set---in a single batch. The held-out prompts are assigned indices 0--448, ensuring stable cross-referencing across experiments.

\paragraph{Why a single combined corpus.} Several of our geometric features (density, centrality, intrinsic dimension) are defined relative to a reference distribution. Embedding all prompts in a single pass guarantees that the reference distribution is identical when comparing geometry across the held-out and training sets. Separate embedding runs would risk subtle distributional shifts if the API model version changed between calls, and would force a choice about which subset to use as the reference. The combined corpus eliminates both concerns.

\paragraph{Self-reference.} The combined corpus serves as its own reference distribution: density is measured relative to all 2,879 prompts, and centrality is measured relative to the corpus mean. This is a deliberate design choice. An external reference corpus (e.g., Wikipedia passages or Common Crawl samples) would introduce a confound: density would measure similarity to the external domain rather than within-benchmark structure. Self-reference asks the right question for our purposes: \emph{how does this prompt relate to other prompts in our benchmark?}

The cost of self-reference is that all geometric features are corpus-dependent. A prompt's density score would change if the corpus composition changed---adding or removing prompts shifts every point's neighborhood. This limits the generalizability of absolute feature values, though relative rankings are more stable. We discuss this further in Section~\ref{sec:embedding-limitations}.

\paragraph{No preliminary dimensionality reduction.} We operate on the full 3,072-dimensional embedding space without applying a global dimensionality reduction step (e.g., PCA or UMAP) as preprocessing. Dimensionality reduction is used within specific feature computations (notably oppositeness, Section~\ref{sec:oppositeness}), but the raw embeddings are preserved for neighbor-based features. This avoids discarding information that might be relevant for local geometric structure.
...
\begin{tikzpicture}[
    scale=1.0,
    real point/.style={circle, fill=black!30, inner sep=1.6pt},
    highlight/.style={circle, fill=blue!70, inner sep=2.2pt},
    opposite/.style={diamond, fill=red!70, inner sep=2.2pt},
    nearest/.style={circle, fill=black!30, draw=red!70, line width=1pt, inner sep=1.6pt},
    >=Stealth
]

% Axes
\draw[->, gray, thin] (-4.2,0) -- (4.2,0) node[right, font=\footnotesize] {PC$_1$};
\draw[->, gray, thin] (0,-2.7) -- (0,3.2) node[above, font=\footnotesize] {PC$_2$};

% Reflection axis (dashed)
\draw[dashed, gray!50, thin] (0,-2.5) -- (0,3.0);

% Real corpus points — dense cluster on right side
\node[real point] at (1.2, 0.8) {};
\node[real point] at (1.8, 1.5) {};
\node[real point] at (2.3, 0.3) {};
\node[real point] at (1.5, -0.4) {};
\node[real point] at (2.8, 1.0) {};
\node[real point] at (1.0, 1.8) {};
\node[real point] at (2.1, 2.0) {};
\node[real point] at (3.0, 0.5) {};
\node[real point] at (1.7, -0.9) {};
\node[real point] at (2.5, 1.8) {};
\node[real point] at (0.8, 0.2) {};
\node[real point] at (1.4, 1.2) {};
\node[real point] at (3.2, 1.4) {};
\node[real point] at (2.0, -0.2) {};
\node[real point] at (0.6, -0.5) {};

% A few sparse points on left side
\node[real point] at (-1.5, -1.8) {};
\node[real point] at (-0.8, 1.5) {};
\node[nearest] (nn) at (-1.8, -0.2) {};  % Nearest real point to the opposite
\node[real point] at (-0.5, -0.8) {};

% The query point (right side)
\node[highlight] (zi) at (2.1, 1.3) {};
\node[right, font=\footnotesize, blue!70] at (2.3, 1.3) {$\mathbf{z}_i$};

% The opposite point (left side — negate PC1, keep PC2)
\node[opposite] (opp) at (-2.1, 1.3) {};
\node[left, font=\footnotesize, red!70] at (-2.3, 1.3) {$\tilde{\mathbf{z}}_i$};

% Arrow showing the flip
\draw[->, thick, dashed, black!60] (zi) -- node[above, font=\footnotesize, pos=0.5, black!80] {negate PC$_1$} (opp);

% Distance to nearest real point
\draw[<->, thick, red!70] (opp) -- node[left, font=\footnotesize, pos=0.45, red!80] {oppositeness} (nn);

% Annotation: dense vs sparse
\draw[decorate, decoration={brace, amplitude=5pt, mirror}, gray] (3.6, -1.2) -- (3.6, 2.3) node[midway, right=6pt, font=\footnotesize, text=gray, align=left] {dense};
\draw[decorate, decoration={brace, amplitude=5pt}, gray] (-3.4, -2.0) -- (-3.4, 1.8) node[midway, left=6pt, font=\footnotesize, text=gray, align=left] {sparse};

\end{tikzpicture}
\caption{Schematic of the oppositeness computation, shown in a simplified two-dimensional PCA space. The query point $\mathbf{z}_i$ (blue circle) is reflected by negating its first principal component, producing the ``opposite'' $\tilde{\mathbf{z}}_i$ (red diamond). The oppositeness score is the cosine distance from $\tilde{\mathbf{z}}_i$ to the nearest real corpus point (circled). When the original point lies in a directionally asymmetric region---dense on one side, sparse on the other---its opposite lands far from any real data, yielding a high oppositeness score. In the actual computation, 10 PCA components are used and the top 3 are negated.}
...
All hyperparameters were fixed during the initial benchmark experiments (Chapter~\ref{ch:geo-predict-hallucination}) and held constant for all subsequent experiments. No hyperparameter tuning was performed on the training set data. The key choices are:
...
% FLAG: Big space again?
\subsection{Limitations}
\label{sec:embedding-limitations}

We note several limitations of the embedding approach, each of which constrains the scope of conclusions that can be drawn from the geometric analysis.

\paragraph{Single embedding model.} All geometric features depend on the choice of embedding model. A preliminary comparison using three embedding models---\texttt{text-embedding-3-small} (1,536 dimensions), \texttt{text-embedding-3-large} (3,072 dimensions), and the open-source \texttt{all-mpnet-base-v2} (768 dimensions)---was conducted on the held-out test set and is reported in Appendix~\ref{app:embedding-robustness}. However, this comparison covers only a subset of features and does not constitute a full robustness analysis. The dependence on a single primary embedding model remains a limitation.

\paragraph{External vs.\ internal representations.} We embed the question text using an external model, not the target models' internal activations. As stated earlier, this tests a different (and arguably stronger) hypothesis---that hallucination risk is encoded in the text itself, not in how a specific model processes it---but it cannot capture model-specific difficulty. A prompt that is geometrically ``normal'' by our features may still be difficult for a particular model due to gaps in that model's training data. Conversely, a geometrically ``anomalous'' prompt may be well-handled by a model that has seen similar examples. The geometric features are best understood as capturing properties of the \emph{question}, not of any specific model's relationship to it.

\paragraph{Corpus dependence.} Self-reference makes all features dependent on corpus composition. Adding or removing prompts changes every point's density, centrality, and neighborhood-based features. Absolute feature values are therefore not portable across different benchmarks. Relative rankings within a fixed corpus are more stable, and all analyses in this thesis use a fixed corpus, but generalization to new prompt distributions would require recomputing features against an appropriate reference.

\paragraph{Static features.} The geometric features are computed once from prompt text before generation. They do not capture dynamics during response generation, such as attention patterns, token-level uncertainty, or the model's evolving internal state. This is by design---pre-generation risk signals are the most practically useful, since they can flag potential hallucinations before any computation is spent on generation---but it means the features cannot distinguish between prompts that are difficult due to knowledge gaps and those that are difficult due to generation-time pathologies (e.g., degenerate sampling).

% Chapter closing
\bigskip

With the benchmark, model selection, judging pipeline, and geometric feature extraction defined, the shared experimental infrastructure is complete. Before turning to results, we briefly note the statistical approach common to all three experimental chapters.

The analyses in Chapters~\ref{ch:geo-predict-hallucination}--\ref{ch:finetuning} use non-parametric tests throughout. Unpaired group comparisons (e.g., geometric feature values for hallucinated versus correct prompts) use the Mann-Whitney U test with rank-biserial correlation as the effect size measure. Paired binary comparisons (e.g., the same prompt under two experimental conditions) use McNemar's test with Yates' continuity correction. When testing multiple hypotheses simultaneously, we apply Bonferroni correction. We prefer non-parametric tests because hallucination labels are categorical, sample sizes vary substantially across prompt categories, and we make no distributional assumptions about the geometric features. Full test specifications and equations are given in each chapter alongside the analyses they support.

The following three chapters each apply this infrastructure to a distinct research question. Chapter~\ref{ch:geo-predict-hallucination} tests whether geometric properties of prompt embeddings predict hallucination across models. Chapter~\ref{ch:prefixes} tests whether targeted prompt prefixes reduce hallucination rates. Chapter~\ref{ch:finetuning} bridges prediction and intervention, testing whether geometric features predict which hallucinations resist mitigation, and presents the full pipeline from geometric diagnosis to fine-tuned correction.
