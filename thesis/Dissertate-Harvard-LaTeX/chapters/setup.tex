...
\paragraph{Limitations of model selection.} We note several constraints. First, API-only models do not disclose parameter counts, precluding a clean scaling analysis. Second, model behavior changes over time as providers update their systems; our results reflect model versions available at the time of evaluation (Fall 2025). Third, ten models cannot exhaustively represent all architectures---notably, we do not include models from Google (Gemini) or non-transformer architectures. Fourth, all open-weight models in our suite use MoE architectures while all API-only models are dense, partially confounding architecture type with access type. This does not affect the geometric analysis, which uses a shared external embedding model (Section~\ref{sec:embeddings}), but it means we cannot cleanly isolate the effect of MoE routing from other provider differences. The set was chosen to be representative rather than exhaustive.
...
\paragraph{Architectural contrast.} As described in Section~\ref{sec:benchmark-models}, the two models differ substantially in architecture and scale: Mixtral uses 8 experts with $\sim$12.9B active parameters, while Llama uses 128 experts with 17B active. They are trained by different organizations (Mistral AI and Meta) with different data and procedures, ensuring that shared patterns---prompts both models hallucinate on, or interventions that help both---are unlikely to reflect idiosyncratic training artifacts.
...
\draw[<->, thick, red!65] (opp) -- node[left=1pt, font=\footnotesize, pos=0.45, red!70] {oppositeness} (nn);

% Annotation: dense vs sparse
\draw[decorate, decoration={brace, amplitude=5pt, mirror}, black!40] (3.8, -0.8) -- (3.8, 2.5) node[midway, right=6pt, font=\footnotesize, text=black!50] {dense};
\draw[decorate, decoration={brace, amplitude=5pt}, black!40] (-4.7, -2.0) -- (-4.7, 1.8) node[midway, left=6pt, font=\footnotesize, text=black!50] {sparse};
...
\paragraph{Single embedding model.} All geometric features depend on the choice of embedding model. A preliminary comparison using three embedding models---\texttt{text-embedding-3-small} (1,536 dimensions), \texttt{text-embedding-3-large} (3,072 dimensions), and the open-source \texttt{all-mpnet-base-v2} (768 dimensions)---is conducted on the held-out test set and reported in Appendix~\ref{app:embedding-robustness}. However, this comparison covers only a subset of features and does not constitute a full robustness analysis. The dependence on a single primary embedding model remains a limitation.

\paragraph{External vs.\ internal representations.} As discussed in Section~\ref{sec:embedding-model}, the use of an external embedding model tests a stronger hypothesis (hallucination risk encoded in question text alone) but cannot capture model-specific difficulty. The geometric features are best understood as capturing properties of the \emph{question}, not of any specific model's relationship to it.

\paragraph{Corpus dependence.} As noted in Section~\ref{sec:corpus-construction}, self-reference makes all features dependent on corpus composition. Absolute feature values are not portable across different benchmarks; generalization to new prompt distributions would require recomputing features against an appropriate reference.
