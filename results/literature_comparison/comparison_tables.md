...
4. **Prefix bridge — V5 replication** [†class imbalance]:
   - 5-fold CV AUC: 0.573 (Mixtral, n=333 fixed vs 27 broken), 0.712 (Llama, n=211 fixed vs 24 broken) — V5, 2,430 prompts, 4 prefixes (CoT excluded)
   - [†] **Caveat**: Mixtral's class imbalance (333:27) limits logistic regression reliability. Llama's AUC of 0.712 indicates genuine predictive signal. Updated Mar 22 after discovering bridge analysis script included CoT (which was excluded from thesis); old numbers had 15/5 broken, inflated by CoT refusals counted as "fixes."
...
5. **Prefix bridge — V5 within-category confirmation** (methodologically strongest):
   - Among hallucinated nonexistent prompts, density significantly predicts fixability:
     - Mixtral: p=0.046, broken density 1.96 vs fixed 2.18 (n=13 broken, 164 fixed)
     - Llama: p=0.012, broken density 1.88 vs fixed 2.15 (n=7 broken, 107 fixed)
   - Llama plausible_fake oppositeness also significant: p=0.009 (n=14 broken, 46 fixed)
   ...
   - [†] **Correction caveat**: 24 within-category tests conducted. Bonferroni correction (α=0.05/24=0.0021) — no individual result survives. However, density was the *predicted* feature from V4 analysis (a priori hypothesis, not exploratory), and the direction is consistent across both models and both prediction levels (hallucination and fixability).
...
V4 pilot (449 prompts, post-contamination-fix) showed lower absolute rates (Entity-Aware Mixtral 1.34%, Structured Caution Llama 1.34%) on a smaller, less diverse prompt set. V5 is the more reliable estimate.
