...
- **k=20** samples per (prompt, model) at T=0.7, top_p=1.0, **max_tokens=2048** — attenuation is driven by k, and cost is not the constraint. max_tokens is set from measured natural completion length, not intuition: the two models differ ~7× at the median (Llama p50=96 tokens, gpt-oss p50=699), and at the original 256 gpt-oss was truncated mid-sentence on most prompts. The judge scores a truncated answer *as* the answer, so an asymmetric truncation rate biases one model's P̂ relative to the other. `finish_reason` is recorded per completion so truncation is measured rather than inferred.
- **Known confound**: the ~7× verbosity gap is prompt-dependent within category (on `nonexistent`, Llama ~90 tokens vs gpt-oss ~1535). A longer answer has more chances to trip the judge's hallucination rule, so spec §6.5.4 residualizes P̂ on per-prompt mean completion length and recomputes τ. If τ_corr collapses under that control, the pilot measured verbosity agreement rather than prompt-difficulty agreement, and must say so.
...
- **Cost**: ~$5–9 generation (Together) + ~$50 judging (Anthropic) ≈ **$55–60**, across two separate bills. Roughly double the initial estimate: completions are longer at max_tokens=2048, and the 628-token judge system prompt is re-sent on all 28,160 calls (~17.7M input tokens on its own). The Anthropic Batch API would halve judging to ~$25 but reworks the §5.1 retry contract — worth it at Phase 1 volume.
- **Wall clock**: plan an overnight run. The 256-token smoke test sustained 1.2 completions/sec; throughput scales roughly inversely with output length.
